...
{
    // TODO
    Program* program = &programs[cur]; //pointer to program structure at the index cur 
    ...
    {
        if (program->fd_in != -1) // fd_in has been specified already 
        {
            dup2(program->fd_in, 0);  //redirects the child's fd_in
            close(program->fd_in); 
        }

        if (program->fd_out != -1) 
        {
            dup2(program->fd_out, 1); 
            close(program->fd_out); 
        }

        execvp(program->argv[0], program->argv); 
        die("Execution of program failed"); 
    }
...
{
    // TODO
    //for loop to iterate through the array and create a pipe for each one 
    int pipefd[2]; 
    for (int i = 0; i < num_programs - 1; i++)
    {
        ...
        programs[i].fd_out = pipefd[1]; //set the fd_out as that of the current program 
        programs[i+1].fd_in = pipefd[0]; //set the fd_in as the next program 
    }
}
