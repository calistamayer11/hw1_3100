...
int transaction(TPlayer *p1, TPlayer *p2, int amount)
{
    if (p1->balance >= amount) 
	{
        ...
        // transfer entire amount of p1 to p2 
...
int one_round(int m, int n, TPlayer p[], TProperty prop[])
{
	for(int i = 0; i < m; i++)
	{
		...
		int new_loc = (current_loc + steps) % n; // get new location of player
		...
		if (prop[new_loc].owner_id == -1) //if player doesn't own the property then they need to pay rent. 
		{
			prop[new_loc].owner_id = p[i].id; 
		}
		else
		{
			int transaction_result = transaction(&p[i], &p[prop[new_loc].owner_id], prop[new_loc].rent);
